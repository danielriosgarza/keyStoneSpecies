...
for env_num in range(1,26):    
    ...
    models = [cobra.io.read_sbml_model(os.path.join(model_folder, i)) for i in os.listdir(model_folder)]
    ...
    filePath = os.path.join(Path(os.getcwd()).parents[0], 'files', '100_env_all_models', str(env_num)+'.tsv')
